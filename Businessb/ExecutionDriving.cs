...
{
    public class ExecutionDriving
    {
        Document _doc;
        public ExecutionDriving(Document doc)
        {
            _doc = doc;
        }
        ...
        public void HazardPlacement(IList<Element> collection)
        {
            ...
            foreach (var item in collection)
            {
                ...
                if (lp != null)
                {
                    ...
                    //loadHazrdModel.LoadHazrd_Driving(_doc, newLocationPoint);
                }
            }
        }
        public void HazardPlacementwindow_driving(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();

            //GetDirection_driving_window
            XYZ lp = GetLocation(collection);
            if (lp != null)
            {
                XYZ newLocationPoint = GetDirection_driving_window(collection, lp);
                loadHazrdModel.LoadHazrd_Driving(_doc, newLocationPoint);
            }
        }
        public void HazardPlacementSF_driving(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();

            //GetDirection_driving_SF
            XYZ lp = GetLocation(collection);
            if (lp != null)
            {
                XYZ newLocationPoint = GetDirection_driving_SF(collection, lp);
                loadHazrdModel.LoadHazrd_Driving(_doc, newLocationPoint);
            }
        }
        ...
        public void HazardPlacementstairs(ICollection<ElementId> collection)
        {
            ...
            foreach (ElementId stairId in collection)
            {
                ...
                foreach (ElementId runId in runIds)
                {
                    ...
                    loadHazrdModel.LoadHazrd_Driving(_doc, lp);
                }
        ...
        /// <returns></returns>
        private XYZ GetDirection_driving_window(Element e, XYZ locationPoint)
        {
            XYZ newPoint = null;
        ...
        private XYZ GetDirection_driving_SF(Element e, XYZ locationPoint)
        {
            XYZ newPoint = null;
            bool gotOrientation = false;
            Sign sign;
