...
{
    public class ExecutionPeoplePlantInterface
    {
        Document _doc;
        public ExecutionPeoplePlantInterface(Document doc)
        {
            _doc = doc;
        }
        /// <summary>
        ...
        public void HazardPlacement(IList<Element> collection)
        {
            ...
            foreach (var item in collection)
            {
                ...
                if (lp != null)
                {
                    ...
                    loadHazrdModel.LoadHazrd_PeoplePlantInterface(_doc, newLocationPoint);
                }
            }
        }
        ...
        public void HazardPlacementPPI_SF(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();
            ...
            if (lp != null)
            {
                XYZ newLocationPoint = GetDirectionPPI_SF(collection, lp);
                loadHazrdModel.LoadHazrd_PeoplePlantInterface(_doc, newLocationPoint);
            }
        }
        //HazardPlacementPPI_FS
        public void HazardPlacementPPI_FS(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();
        ...
        //HazardPlacementPPI_FC
        public void HazardPlacementPPI_FC(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();
        ...
        //HazardPlacementframePPI_STEEL
        public void HazardPlacementframePPI_STEEL(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();
            XYZ lp = null;
        ...
        private XYZ GetLocationcentroidFS(Element e)
        {
            ...
            if (e is HostObject)
            {
                ...
                foreach (GeometryObject geomObj in geom)
                {
                    ...
                    newPoint = new XYZ(lp.X + 10, lp.Y, lp.Z);
        ...
        //HazardPlacementPPI_SF
        public void HazardPlacementPPI_pile(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();
            ...
            if (lp != null)
            {
                XYZ newLocationPoint = GetDirectionPPI_pile(collection, lp);
                loadHazrdModel.LoadHazrd_PeoplePlantInterface(_doc, newLocationPoint);
            }
        }
        ...
        public void HazardPlacementstairs(ICollection<ElementId> collection)
        {
            ...
            foreach (ElementId stairId in collection)
            {
                ...
                foreach (ElementId runId in runIds)
                {
                    ...
                    loadHazrdModel.LoadHazrd_PeoplePlantInterface(_doc, lp);
                }
        ...
        private XYZ GetDirectionPPI_pile(Element e, XYZ locationPoint)
        {
            XYZ newPoint = null;
            bool gotOrientation = false;
            Sign sign;
            ...
            if (e is FamilyInstance)
            {
                ...
                {
                    if (sign == Sign.Positive)
                        newPoint = new XYZ(locationPoint.X - 13, locationPoint.Y, locationPoint.Z);
                    else if (sign == Sign.Negative)
                        newPoint = new XYZ(locationPoint.X + 13, locationPoint.Y, locationPoint.Z);
                ...
                {
                    if (sign == Sign.Positive)
                        newPoint = new XYZ(locationPoint.X, locationPoint.Y - 17, locationPoint.Z);
                    else if (sign == Sign.Negative)
                        newPoint = new XYZ(locationPoint.X, locationPoint.Y + 17, locationPoint.Z);
                ...
                if (!gotOrientation)
                {
                    ...
                    if (sign == Sign.Positive)
                        newPoint = new XYZ(locationPoint.X - 17.5, locationPoint.Y, locationPoint.Z);
                    else if (sign == Sign.Negative)
                        newPoint = new XYZ(locationPoint.X + 17.5, locationPoint.Y, locationPoint.Z);
                }
            }
        ...
        private XYZ GetDirectionPPI_SF(Element e, XYZ locationPoint)
        {
            XYZ newPoint = null;
            bool gotOrientation = false;
            Sign sign;
