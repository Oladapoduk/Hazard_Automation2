...
{
    public class ExecutingElectricity
    {

        Document _doc;
        public ExecutingElectricity(Document doc)
        {
            _doc = doc;
        }
        public void HazardPlacement(IList<Element> collection)
        {
            ...
            foreach (var item in collection)
            {
                ...
                if (lp != null)
                {
                    ...
                    //loadHazrdModel.LoadHazrd_Electricity(_doc, newLocationPoint);
                }
            }
        }
        ...
        public void HazardPlacementElectricity_pile(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();

            // GetDirection_pile_electric
            XYZ lp = GetLocation(collection);
            if (lp != null)
            {
                XYZ newLocationPoint = GetDirection_pile_electric(collection, lp);
                loadHazrdModel.LoadHazrd_Electricity(_doc, newLocationPoint);
            }
        }
        public void HazardPlacementElectricity_SF(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();
            ...
            if (lp != null)
            {
                XYZ newLocationPoint = GetDirection_SF_electric(collection, lp);
                loadHazrdModel.LoadHazrd_Electricity(_doc, newLocationPoint);
            }
        }
        //HazardPlacementElectricity_FC
        public void HazardPlacementElectricity_FC(Element collection)
        {
            LoadHazrdModel loadHazrdModel = new LoadHazrdModel();
        ...
        private XYZ GetLocationcentroidFS(Element e)
        {
            ...
            if (e is HostObject)
            {
                ...
                foreach (GeometryObject geomObj in geom)
                {
                    ...
                    newPoint = new XYZ(lp.X , lp.Y+35, lp.Z);
        ...
        public void HazardPlacementstairs(ICollection<ElementId> collection)
        {
            ...
            foreach (ElementId stairId in collection)
            {
                ...
                foreach (ElementId runId in runIds)
                {
                    ...
                    loadHazrdModel.LoadHazrd_Electricity(_doc, lp);
                }
        ...
        /// <returns></returns>
        private XYZ GetDirection_pile_electric(Element e, XYZ locationPoint)
        {
            XYZ newPoint = null;
            ...
            if (e is FamilyInstance)
            {
                ...
                if (GetOrientation(fi.FacingOrientation.X, out sign))
                {
                    if (sign == Sign.Positive)
                        newPoint = new XYZ(locationPoint.X - 28, locationPoint.Y, locationPoint.Z);
                ...
                {
                    if (sign == Sign.Positive)
                        newPoint = new XYZ(locationPoint.X, locationPoint.Y - 38, locationPoint.Z);
                    else if (sign == Sign.Negative)
                        newPoint = new XYZ(locationPoint.X, locationPoint.Y + 38, locationPoint.Z);
                ...
                if (!gotOrientation)
                {
                    ...
                    if (sign == Sign.Positive)
                        newPoint = new XYZ(locationPoint.X - 38.5, locationPoint.Y, locationPoint.Z);
                    else if (sign == Sign.Negative)
                        newPoint = new XYZ(locationPoint.X + 38.5, locationPoint.Y, locationPoint.Z);
                }
            }
        ...
        private XYZ GetDirection_SF_electric(Element e, XYZ locationPoint)
        {
            XYZ newPoint = null;
            bool gotOrientation = false;
            Sign sign;
        ...
        private XYZ GetDirectionHealth(Element e, XYZ locationPoint)
        {
            XYZ newPoint = null;
            bool gotOrientation = false;
            Sign sign;
            ...
            if (e is FamilyInstance)
            {
                ...
                {
                    if (sign == Sign.Positive)
                        newPoint = new XYZ(locationPoint.X - 5, locationPoint.Y, locationPoint.Z);
                    else if (sign == Sign.Negative)
                        newPoint = new XYZ(locationPoint.X + 5, locationPoint.Y, locationPoint.Z);
